...
def analyze_player(split_dict, split_category, output_directory):
    all_results = []
    if split_category == 'all':
        for key in split_dict.keys():
            result = analyze_split(key, split_dict[key], output_directory)
            if len(result) > 0:
                all_results.append(result)

    elif split_category in split_dict.keys():
        result = analyze_split(split_category, split_dict[split_category], output_directory)
        if len(result) > 0:
            all_results.append(result)
...
def analyze_split(split_name, split_df, outdir = 'Results'):
    ...
    with open('{}/{}.txt'.format(outdir, player_name), 'w') as f:
        f.write(text_string)
        f.close()
...
if __name__ == "__main__":
    ...
    for split_dict in basketball_dicts:
        analyze_player(split_dict, args.split_category, args.outdir)
